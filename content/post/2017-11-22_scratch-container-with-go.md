...
## Go 바이너리 컨테이너
앞서 말했다시피 Go 는 바이너리만으로 실행이 가능하다.[^1] Go 언어로 작성된 Docker 특성상 Go 언어와의 궁함이 좋을 수 밖에 없는 것 같다. 위에서 작성한 `Hello, World!` 의 기능을 하는 `main.go` 코드는 아래와 같다.
...
이제 빌드를 해야 한다. `CGO_ENABLED` 환경변수를 **0** 으로 설정해서 cgo 가 아닌 Go 컴파일러를 사용하도록 해야 한다. `-a` Flag 는 Go 에 모든 종속성을 다시 빌드하도록 한다. 그렇지 않으면 여전히 동적으로 연결된 종속성을 가지기 때문이다. 마지막으로 Linker 와 관련하여 `-ldflags '-s'` 와 같이 빌드 옵션을 부여해서 실행 파일의 크기를 약 50% 줄이는 것이 좋다.[^2]
...
[^2]: 심볼 테이블 제거 등 결과물을 작게 만든다. `man ld` 로 검색해서 보면 Obsolete 되었다고 나오긴 하지만 Go 빌드 옵션에서는 여전히 유효한 것 같다.
